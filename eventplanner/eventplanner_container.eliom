...
let%server uploader () = Eba_userbox.uploader !Eventplanner_config.avatar_dir
...
let header ?user () =
  ...
  Lwt.return
    (header ~a:[a_id "main"] [
       a ~a:[a_id "eventplanner-logo"]
         ~service:Eba_services.main_service [
         pcdata Eventplanner_base.application_name;
       ] ();
       ul ~a:[a_id "eventplanner-navbar"]
         [
           li [a ~service:Eba_services.main_service
                 [pcdata "Home"] ()];
           li [a ~service:Eventplanner_services.about_service
                 [pcdata "About"] ()]
         ];
       user_box;
     ])
...
let footer ?user () =
  div ~a:[a_id "eventplanner-footer"] [
...
let%server page userid_o content =
  ...
  in
  let l = [
    div ~a:[a_id "eventplanner-body"] content;
    footer ();
  ] in
